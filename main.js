...
module.exports = new Proxy(new Function(),{
  ...
  set: function(target,prop,value){
    if(Object.keys(settings).includes(prop)){
      if(typeof settings[prop] == typeof value){
        settings[prop] = value
        if(prop == 'callLimit'){
          if(queue.queue == 0){
            queue = promiseLimit(value)
          } else {
        ...
        throw new Error(prop+" was expecting a "+typeof settings[prop]+" instead of a "+typeof value)
      }
    } else {
      throw new Error("I don't have a setting with the name "+prop)
    }
  }
})